...
import org.apache.hadoop.hive.ql.optimizer.correlation.InlineOptimizer;
import org.apache.hadoop.hive.ql.optimizer.correlation.CorrelationOptimizer;
import org.apache.hadoop.hive.ql.optimizer.correlation.ReduceSinkDeDuplication;
import org.apache.hadoop.hive.ql.optimizer.correlation.RelaxCorrelationOptimizer;
...
public class Optimizer {
  ...
  public void initialize(HiveConf hiveConf) {
    ...
    if (HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVEMULTIQUERY) &&
        HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVEINLINE)) {
        transformations.add(new InlineOptimizer());
    }
    if(HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVEOPTCORRELATION) &&
      ...
      if(HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVEMULTIQUERY)){
        //Comment out JFC optimizer to make TC optimizer work (Just for test !)
        //transformations.add(new InlineOptimizer());
        // TODO: create a new confVar for relaxed correlation optimizer
        // TODO: write a new phase for this optimizer when multi-query option is on
        transformations.add(new CorrelationOptimizer());
        transformations.add(new RelaxCorrelationOptimizer());
      }else{
        transformations.add(new CorrelationOptimizer());
      }
      // Comment out JFC optimizer to make TC optimizer work (Just for test !)
