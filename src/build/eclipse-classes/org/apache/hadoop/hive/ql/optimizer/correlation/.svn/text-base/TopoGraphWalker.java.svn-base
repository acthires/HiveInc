...
/**
 * Bottom-up topo-order operator graph walker this class takes list of starting ops
 ...
 * dispatched
 */
public class TopoGraphWalker implements GraphWalker {
   ...
   */
  public TopoGraphWalker(Dispatcher disp) {
    dispatcher = disp;
  ...
  public void walk(Node nd) throws SemanticException {
    ...
    if ((op.getParentOperators() == null)
        || getDispatchedList().containsAll(op.getParentOperators())) {
      // all children are done or no need to walk the children
      if (!getDispatchedList().contains(nd)) {
        dispatch(nd, opStack);

        if (nd.getChildren() != null) {
          // At this point, none of the children should be dispatched yet
    ...
    // add self to the end of the queue in that order
    getToWalk().remove(nd);
    getToWalk().add(nd);
  }
}
