...
public class CliDriver {
  ...
  public static final int LINES_TO_FETCH = 40; // number of lines to fetch in batch from remote hive
                                               // server

  public static final String HIVERCFILE = ".hiverc";
  public final ArrayList<Pair<String, Configuration>> multiCmds = new ArrayList<Pair<String, Configuration>>();
  ...
  // this function only support a common client( have the same SessionState) and query sql
  // running in the loacl model
  public int processMultiCmds() {
    int ret = 0;
    CliSessionState ss = (CliSessionState) SessionState.get();
    ss.err.flush();

    MultiDriver drv = new MultiDriver();
    ret = processLocalMultiCmd( drv, ss);
  ...
  int processLocalMultiCmd(MultiDriver qp, CliSessionState ss) {
    int tryCount = 0;
    boolean needRetry;
    int ret = 0;
    ...
    do {
      try {
        ...
        if (qp != null) {
            ...
            long start = System.currentTimeMillis();
            qp.setTryCount(tryCount);
            ret = qp.multiRun(multiCmds).getResponseCode();
            if (ret != 0) {
              qp.close();
              return ret;
            }
  ...
  private void printHeader(MultiDriver qp, PrintStream out) {
    for (Schema schema : qp.getSchemas()) {
  ...
  public int processLine(String line, boolean allowInterupting) {
    ...
    boolean validMultiQuery = false;
    validMultiQuery = ((HiveConf) conf).getBoolVar(HiveConf.ConfVars.HIVEMULTIQUERY);
    int multiQueryNum = ((HiveConf) conf).getIntVar(HiveConf.ConfVars.HIVEMULTIQUERYNUM);
    try {
      int lastRet = 0, ret = 0, multiQuerycounter = 0;
      String command = "";
      for (String oneCmd : line.split(";")) {
        ...
        if(validMultiQuery){
          // 1 Flag.MultiQuery is on
          if(isDDLQuery(command)){
            // 1.1 ordinary query (select || from || insert)
            if( multiQuerycounter >= multiQueryNum){
              ret = processMultiCmds();
              this.multiCmds.clear();
              multiQuerycounter=0;
            }
            this.multiCmds.add(new Pair<String, Configuration>(command, (HiveConf) conf));
            multiQuerycounter++;
          }else if(isAggrDML(command)){
            //1.2 CTAS
            ret = processCmd(command);
          }else{
            // 1.3 default
            if(multiQuerycounter>0){
              ret = processMultiCmds();
              this.multiCmds.clear();
              multiQuerycounter=0;
            }
      ...
      if (validMultiQuery && multiQuerycounter > 0) {
        lastRet = processMultiCmds();
        this.multiCmds.clear();
      }
      CommandProcessorFactory.clean((HiveConf) conf);
      return lastRet;
   ...
   *
   * @param ss
   *          CliSessionState of the CLI driver
   * @param conf
   *          HiveConf for the driver sionssion
   * @param oproc
   *          Opetion processor of the CLI invocation
  ...
  public boolean isDDLQuery(String cmd) {
    String cmd_trimmed = cmd.trim();
    String cmd_tolower =cmd_trimmed.toLowerCase();
    String[] tokens = tokenizeCmd(cmd_tolower);
    if (tokens[0].equals("select") ||
        tokens[0].equals("from")||
        tokens[0].equals("insert")) {
      return true;
    } else {
      if (tokens[0].equals("create")) {
        for(int i=1;i<tokens.length-1;i++){
          if(tokens[i].equals("as")&&tokens[i+1].equals("select")){
            return true;
          }
        }
  ...
  public boolean isAggrDML(String cmd) {
    String cmd_trimmed = cmd.trim();
    String[] tokens = tokenizeCmd(cmd_trimmed);
