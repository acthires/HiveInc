...
import org.apache.hadoop.hive.ql.parse.CommonSubtreeReuse;
...
public class MultiDriver  implements CommandProcessor {
  ...
  // new variable for multi query
  private final ArrayList<Schema> schemas = new ArrayList<Schema>();
  private final ArrayList<String> cmds = new ArrayList<String>();
  private final ArrayList<HiveConf> hconfs = new ArrayList<HiveConf>();
  private int multiQueryNum;
  private final HashMap<Integer, ParseContext> multiPctx = new HashMap<Integer, ParseContext>();
   ...
   */
  public MultiDriver(HiveConf conf) {
    this.conf = conf;
  }
   ...
   */
  public int multiCompile(ArrayList<Pair<String, Configuration>> multiCmds) {
 /*   if(((HiveConf) conf).getBoolVar(HiveConf.ConfVars.HIVEINC)){
      return incrementalCompile(multiCmds,true);
    }else{
      return multiCompile(multiCmds, true);
    }*/
    return multiCompile(multiCmds, true);
  }
   ...
   * @return 0 for ok
   */
  public int multiCompile(ArrayList<Pair<String, Configuration>> multiCmds, boolean resetTaskIds) {
    ...
    try {
      for (int i = 0; i < multiCmds.size(); i++) {
        ...
        Pair<String, Configuration> cmdConf = multiCmds.get(i);
        ...
        LOG.info("Multi Semantic Analysis Phase1 Completed");
      ...
      // Optree output
      LOG.info("======Print Multi Query Optree After sem.analyzePhase1 =======");
      ...
      InterQueryFlowCtx queryFlowCtx = new InterQueryFlowCtx(conf, multiPctx);
  ...
  public static  void addbottomfakeoperator(MultiParseContext multipCtx,HashSet<Integer> set) {
    ...
    List<Operator<? extends OperatorDesc>> oplistRS = new ArrayList<Operator<? extends OperatorDesc>>() ;
    HashMap<Integer, ParseContext> multipctx=multipCtx.getmultipctx();
    for(int i=0;i<multipctx.size();i++){
      if(!set.contains(i)){
        Iterator<String> it=multipctx.get(i).getTopOps().keySet().iterator();
        if(it.hasNext()){
          String key= it.next();
          genbottomopFSlist(oplistFS,multipctx.get(i).getTopOps().get(key));
        }
    ...
    for(int i=0;i<oplistFS.size();i++){
      genbottomopRSlist(oplistRS,oplistFS.get(i));
    }


    FakeDesc FD=new FakeDesc();
    //RowResolver rs =new RowResolver();
    ArrayList<ColumnInfo> vecCol = new ArrayList<ColumnInfo>();
    RowSchema rs= new RowSchema(vecCol);
    FakeOperator FO=(FakeOperator)OperatorFactory.getAndMakeChild(FD,rs, oplistRS);
   // FakeOperator FO=(FakeOperator)OperatorFactory.getAndMakeChild(FD,rs, oplistFS);
    multipCtx.setFakeOperator(FO);

  }
  public static  Operator<? extends OperatorDesc> getOneTSbyFS(Operator<? extends OperatorDesc> op) {
    if(op instanceof TableScanOperator){
      return op;
    }
    if(op.getParentOperators()!=null&&op.getParentOperators().size()>0){
      return getOneTSbyFS(op.getParentOperators().get(0));
    }
    return null;

  }

  private static void genbottomopRSlist(List<Operator<? extends OperatorDesc>> oplistRS,
     Operator<? extends OperatorDesc> opFS) {
    // TODO Auto-generated method stub
    //Operator<? extends OperatorDesc> fs =oplistFS.get(i);
    if(opFS.getParentOperators()!=null){
      /*
      if(opFS.getParentOperators().size()==1){
        Operator<? extends OperatorDesc> current = opFS.getParentOperators().get(0);
        if(current instanceof ReduceSinkOperator){
          oplistRS.add(current);
          return;
        }else{
          genbottomopRSlist(oplistRS,current);
        }
      }
      */
      for(Operator<? extends OperatorDesc> current:opFS.getParentOperators()){
  ...
  private static void genbottomopFSlist(List<Operator<? extends OperatorDesc>> oplist,
    Operator<? extends OperatorDesc> topop) {
      if(topop.getChildOperators()==null||topop.getChildOperators().size()==0){
        if(topop instanceof TableScanOperator){
          return;
        }
        oplist.add(topop);
      }else{
        genbottomopFSlist(oplist,topop.getChildOperators().get(0));
      }

  }


  private MultiParseContext mergePctx() {
  ...
  public CommandProcessorResponse multiRun(ArrayList<Pair<String, Configuration>> multiCmds)
      throws CommandNeedRetryException {

    CommandProcessorResponse cpr = runInternal(multiCmds);
  ...
  private int createcachetable(ArrayList<Pair<String, Configuration>> multiCmds) throws CommandNeedRetryException {
  ...
  private int CreateTableCompile(ArrayList<Pair<String, Configuration>> multiCmds, boolean resetTaskIds) {
  ...
  public int multiExecute() throws CommandNeedRetryException {
   return execute();
  }
