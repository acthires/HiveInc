...
public class GenMRRedSink2 implements NodeProcessor {
  ...
  public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx opProcCtx,
    ...
    if(op.getStoreDirName() != null){
      Log.info("Find join/gby input cache point in taskDAG. DirName is " + op.getStoreDirName());
    ...
    Log.info("Record rsop " + op.toString() + "'s GBYTaskId " + ctx.getCurrTask().getId());
    op.setTaskId(ctx.getCurrTask().getId());
