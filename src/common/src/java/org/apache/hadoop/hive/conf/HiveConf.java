...
//  private final ArrayList<ParseContext> multiPctx = new ArrayList<ParseContext>();

  //public ArrayList<ParseContext> getMultiPctx() {
  //  return multiPctx;
 // }
  ...
  public static enum ConfVars {
    ...
    HIVEINCRUNORIGINAL("hive.inc.test.runoriginal",false),
    HIVEINCRUNINC("hive.inc.test.runinc",true),
    HIVEINCFINDNEWFILES("hive.inc.test.findNewFiles",true),
    HIVEINCPUTNEWFILES("hive.inc.test.putNewFiles",true),
    HIVEINCFILESOURCEDIR("hive.inc.file.sourcedir","/home/kangyanli/tpch_2_15.0/dbgen/10G/"),
    HIVEINCFILESINKDIR("hive.inc.file.sinkdir","/home/kangyanli/tpch/10G/"),
    HIVEINCSPLITGBY("hive.inc.split.gby",true),
    HIVEINCGBYCOSTMODEL("hive.inc.costmodel.gby",true),
    HIVEINCTESTCOSTMODELSPLIT("hive.inc.test.costmodel.split",false),
    // QL execution stuff
    SCRIPTWRAPPER("hive.exec.script.wrapper", null),
    PLAN("hive.exec.plan", ""),
    SCRATCHDIR("hive.exec.scratchdir", "/tmp/hive-" + System.getProperty("user.name")),
    LOCALSCRATCHDIR("hive.exec.local.scratchdir", System.getProperty("java.io.tmpdir")
    ...
    HIVEHISTORYFILELOC("hive.querylog.location", System.getProperty("java.io.tmpdir")
        + File.separator + System.getProperty("user.name")),
    ...
    HIVE_COMBINE_INPUT_FORMAT_SUPPORTS_SPLITTABLE(
    ...
    HIVEINDEXAUTOUPDATE("hive.optimize.index.autoupdate", false), // automatically update stale
                                                                  // indexes
    ...
    HIVEOPTSORTMERGEBUCKETMAPJOIN("hive.optimize.bucketmapjoin.sortedmerge", false), // try to use
                                                                                     // sorted merge
                                                                                     // bucket map
                                                                                     // join
    ...
    HIVEINLINE("hive.optimize.inline", false), // exploit inter-query inlining
    HIVEOPTCSQ("hive.optimize.csq", false), // exploit inter-query common sub query
    ...
    // Subset of counters that should be of interest for hive.client.stats.publishers (when one
    // wants to limit their publishing). Non-display names should be used".
    ...
    HIVE_LOCK_MANAGER("hive.lock.manager",
    ...
    // temporary variable for testing. This is added just to turn off this feature in case of a bug
    // in
    // deployment. It has not been documented in hive-default.xml intentionally, this should be
    // removed
